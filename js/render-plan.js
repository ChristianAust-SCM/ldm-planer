...
export const DEFS = `<defs>`
  ...
  + `<rect width="7" height="7" fill="#F7F9FA"/>`
  + `<line x1="0" y1="0" x2="0" y2="7" stroke="#D7DEE5" stroke-width="2"/>`
  + `</pattern>`
...
const FZG_LINIE = '#8A9AA8'
const FZG_FLAECHE = '#EAEEF2'
const LADEFLAECHE = '#FFFFFF'
...
function kabineOben(H) {
  ...
  return `<g>
    ...
    <rect x="28" y="${y0.toFixed(1)}" width="108" height="${kb.toFixed(1)}" rx="10" fill="${FZG_FLAECHE}" stroke="${FZG_LINIE}" stroke-width="1.4" opacity="1"/>
  ...
  for (const r of t.reihen) {
    ...
    if (genutzt < breiteM - 0.06) {
      koerper += `<rect class="hatch" x="${(x * PX + 1).toFixed(1)}" y="${(genutzt * PX + 1).toFixed(1)}" `
        ...
        + `rx="2.5" fill="url(#frei)" stroke="#C2CCD6" stroke-width="1"/>`
    }
    x += r.tief
  }
  if (t.rest > 0.005) {
    koerper += `<rect class="hatch" x="${(x * PX + 1).toFixed(1)}" y="1" width="${(t.rest * PX - 2).toFixed(1)}" `
      + `height="${(H - 2).toFixed(1)}" rx="2.5" fill="url(#frei)" stroke="#C2CCD6" stroke-width="1"/>`
    if (t.rest * PX > 34) {
      koerper += `<text x="${((x + t.rest / 2) * PX).toFixed(1)}" y="${(H / 2 + 4).toFixed(1)}" text-anchor="middle" class="fr">frei</text>`
    }
  }
  const oben = 10
  return `<svg viewBox="0 0 962 ${(H + oben + 40).toFixed(0)}" width="100%"
    ...
    <g transform="translate(0,${oben})">
      ...
      <line x1="150" y1="${(H + 14).toFixed(1)}" x2="950" y2="${(H + 14).toFixed(1)}" stroke="#D7DEE5" stroke-width="1"/>
...
export function svgSeite(t, kapa, hoeheM) {
  ...
  if (t.rest > 0.005) {
    koerper += `<rect class="hatch" x="${(x * PX + 1).toFixed(1)}" y="${(oben + 1).toFixed(1)}" width="${(t.rest * PX - 2).toFixed(1)}" `
      + `height="${(H - 2).toFixed(1)}" rx="2" fill="url(#frei)" stroke="#C2CCD6" stroke-width="1"/>`
  }

  const kt = oben + 6, kb = boca - 5
  const rad = cx => `<circle cx="${cx}" cy="${(boca - 14).toFixed(1)}" r="14" fill="${FZG_FLAECHE}" stroke="${FZG_LINIE}" stroke-width="2.5" opacity="1"/>`
    + `<circle cx="${cx}" cy="${(boca - 14).toFixed(1)}" r="4.5" fill="${FZG_LINIE}" opacity=".7"/>`
  return `<svg viewBox="0 0 962 ${(boca + 16).toFixed(0)}" width="100%"
    preserveAspectRatio="xMidYMid meet" role="img"
    ...
    <path d="M 28 ${kb.toFixed(1)} L 28 ${(kt + 20).toFixed(1)} Q 28 ${kt.toFixed(1)} 48 ${kt.toFixed(1)} L 126 ${kt.toFixed(1)} L 136 ${kb.toFixed(1)} Z"
      fill="${FZG_FLAECHE}" stroke="${FZG_LINIE}" stroke-width="1.4" opacity="1"/>
    ...
    <line x1="14" y1="${boca.toFixed(1)}" x2="948" y2="${boca.toFixed(1)}" stroke="#D7DEE5" stroke-width="1.5"/>
  </svg>`
}
